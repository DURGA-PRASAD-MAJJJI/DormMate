...
const Menu = () => {
    ...
    // Automatically cycle through cards (slow animation)
    useEffect(() => {
        if (isPaused) return;
    ...
    return (
        <div className="fixed bottom-10 right-10 w-56 h-80"> 
            {/* Animated Cards (Only one visible at a time) */}
            {!selectedMovie && (
                <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                    <motion.div
                        key={movies[currentIndex].id}
                        className="absolute w-full h-full cursor-pointer border-2 border-primary shadow-lg rounded-lg"
            ...
            {/* Modal for Description (Hides other cards) */}
            {selectedMovie && (
                <motion.div

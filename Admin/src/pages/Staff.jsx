...
const Staff = () => {
  ...
  const filteredStaff = staffMembers.filter(staff => {
    ...
    const matchesSearch = staff.name.toLowerCase().includes(searchLower) || staff.email.toLowerCase().includes(searchLower) || staff.mobile.includes(searchTerm);
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 px-4 sm:px-6 md:pl-[280px] py-8">
      <div className="max-w-8xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowAddForm(true)} className="flex items-center gap-2 bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 text-white px-5 py-2.5 rounded-xl shadow-md transition-all">
        ...
        {filteredStaff.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredStaff.map((staff) => <StaffCard key={staff._id || staff.id} staff={staff} onClick={() => setSelectedStaff(staff)} />)}
          </div>
        ) : (
        ...
        {showAddForm && <AddStaff show={showAddForm} onClose={() => setShowAddForm(false)} onSubmit={handleAddStaff} formData={newStaff} onFormChange={handleFormChange} departments={departments.filter(d => d !== 'all')} />}
      </div>
    </div>
  );
};
